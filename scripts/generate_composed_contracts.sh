...
# run: go run ../cli.go -name <parent_stream> -import <1st stream>:<1st weight>,<2nd stream>:<2nd weight> -out temp_composed_stream_contracts/<parent_stream>.json
...
rm -rf ./temp_composed_stream_contracts

# Creates the directory if it doesn't exist
mkdir -p ./temp_composed_stream_contracts
...
for parent_stream in "${!stream_groups[@]}"; do
    ...
    go run ./contract_gen/cli/cli.go -name "$parent_stream" -import "$streams_weights" -out "./temp_composed_stream_contracts/$parent_stream.json"
done

echo "Composed Contracts Generated"
